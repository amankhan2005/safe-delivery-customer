...
export default function VerifyEmailOTPScreen({ navigation, route }) {
  ...
  const handleKeyPress = ({ nativeEvent }, idx) => {
    if (nativeEvent.key === 'Backspace' && !otp[idx] && idx > 0) inputs.current[idx - 1]?.focus();
  };
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const res = await verifyEmailOTP({ phone, email, otp: code });
  ...
  return (
    <Screen bg={COLORS.white}>
      ...
      <View style={styles.iconBox}><Ionicons name="mail-outline" size={40} color={COLORS.primary} /></View>
      <Text style={styles.title}>Verify Email</Text>
      <Text style={styles.sub}>4-digit code sent to{'\n'}<Text style={styles.bold}>{email}</Text></Text>
      <View style={styles.otpRow}>
        {otp.map((digit, i) => (
          <TextInput key={i} ref={(r) => (inputs.current[i] = r)}
            ...
            keyboardType="number-pad" maxLength={1} textAlign="center" selectionColor={COLORS.primary}
          />
        ))}
      </View>
      <Button title="Verify Email & Create Account" onPress={handleVerify} loading={loading} size="lg" style={styles.btn} />
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 72, height: 72, borderRadius: 20, backgroundColor: COLORS.primaryLight, alignItems: 'center', justifyContent: 'center', marginBottom: SIZES.xl },
  ...
  otpBox: { width: 56, height: 64, borderWidth: 1.5, borderColor: COLORS.border, borderRadius: SIZES.radiusMd, fontSize: 24, fontWeight: '700', color: COLORS.gray900, backgroundColor: COLORS.white },
