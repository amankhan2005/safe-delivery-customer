...
import {
  ...
  Animated, Easing, Dimensions, ActivityIndicator, ScrollView,
} from 'react-native';
...
import { isValidCoord, safeNum, safeCoord, safeMarkerCoord, buildTrackingRegion } from '../utils/safeCoords';
...
// Searching ripple — unchanged from original
...
// SafeMap — map modules loaded inside useMemo (never at module scope)
function SafeMap({ mapRef, mapRegion, pickupLat, pickupLng, dropLat, dropLng, riderCoords, onMapReady }) {
  const [MapViewModule, MarkerModule, PolylineModule, PROVIDER_GOOGLE] = useMemo(() => {
    try { const m = require('react-native-maps'); return [m.default, m.Marker, m.Polyline, m.PROVIDER_GOOGLE]; }
    catch (_) { return [null, null, null, null]; }
  }, []);

  if (!MapViewModule) return (
    <View style={[S.map, { alignItems: 'center', justifyContent: 'center', backgroundColor: '#e8f0fe' }]}>
      <Text style={{ color: '#6B7280', fontSize: 13 }}>Map unavailable</Text>
    </View>
  );

  if (!mapRegion || !isValidCoord(safeNum(mapRegion.latitude), safeNum(mapRegion.longitude))) return (
    <View style={[S.map, { alignItems: 'center', justifyContent: 'center', backgroundColor: '#e8f0fe' }]}>
      <ActivityIndicator color={BLUE} />
    </View>
  );

  const pickupCoord = safeMarkerCoord(safeNum(pickupLat), safeNum(pickupLng));
  const dropCoord   = safeMarkerCoord(safeNum(dropLat),   safeNum(dropLng));
  const riderSafe   = safeCoord(riderCoords);
  const riderCoord  = riderSafe ? safeMarkerCoord(riderSafe.lat, riderSafe.lng) : null;

  const validRegion = {
    latitude:       Number(mapRegion.latitude),
    longitude:      Number(mapRegion.longitude),
    latitudeDelta:  Number(mapRegion.latitudeDelta)  || 0.015,
    longitudeDelta: Number(mapRegion.longitudeDelta) || 0.015,
  };
  ...
  return (
    ...
    >
      {pickupCoord && MarkerModule && (
        <MarkerModule coordinate={pickupCoord} title="Pickup" tracksViewChanges={false}>
          <View style={S.markerPickup}><Ionicons name="location" size={20} color={GREEN} /></View>
        </MarkerModule>
      )}
      {dropCoord && MarkerModule && (
        <MarkerModule coordinate={dropCoord} title="Drop" tracksViewChanges={false}>
          <View style={S.markerDrop}><Ionicons name="location" size={20} color={RED} /></View>
        </MarkerModule>
      )}
      {riderCoord && MarkerModule && (
        <MarkerModule coordinate={riderCoord} title="Rider" tracksViewChanges={false}>
          <View style={S.markerRider}><Ionicons name="bicycle-outline" size={20} color={BLUE} /></View>
        </MarkerModule>
      )}
      {riderCoord && pickupCoord && PolylineModule && (
        <PolylineModule
          coordinates={[
            { latitude: riderCoord.latitude,  longitude: riderCoord.longitude },
            { latitude: pickupCoord.latitude, longitude: pickupCoord.longitude },
            ...(dropCoord ? [{ latitude: dropCoord.latitude, longitude: dropCoord.longitude }] : []),
          ]}
...
export default function LiveTrackingModal({ orderId, visible, onClose, onDelivered, navigation }) {
  ...
  const lastAnimRef = useRef(0); // throttle animateToRegion
  ...
  useEffect(() => {
    ...
    const tick = () => { if (mountedRef.current) setCancelSecs(Math.max(120 - Math.floor((Date.now() - created) / 1000), 0)); };
    tick();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isValidCoord(rLat, rLng)) setRiderCoords({ lat: Number(rLat), lng: Number(rLng) });
      if (['assigned', 'picked_up', 'in_transit'].includes(o.status)) setShowMap(true);
      if (o.status === 'delivered') setTimeout(() => { if (mountedRef.current) onDelivered?.(o); }, 800);
    } catch (_) {}
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        unsubRef.current.location = subscribeRiderLocation(orderId, (data) => {
          if (!mountedRef.current) return;
          const lat = safeNum(data?.lat);
          const lng = safeNum(data?.lng);
          if (!isValidCoord(lat, lng)) return;
          setRiderCoords({ lat: Number(lat), lng: Number(lng) });
          setShowMap(true);
          // Throttle: max 1 animate per 2 seconds
          const now = Date.now();
          if (now - lastAnimRef.current < 2000) return;
          lastAnimRef.current = now;
          if (!mapRef.current || !mapReady || !mountedRef.current) return;
          try {
            mapRef.current.animateToRegion(
              { latitude: Number(lat), longitude: Number(lng), latitudeDelta: 0.015, longitudeDelta: 0.015 },
              600,
            );
          } catch (_) {}
        });
    ...
    return () => {
      ...
      try { unsubRef.current.events?.(); }   catch (_) {}
      try { unsubRef.current.location?.(); } catch (_) {}
      unsubRef.current = { events: null, location: null };
    };
  }, [visible, orderId, load, mapReady]);
  ...
  const cancelProg = Math.max(0, Math.min(1, (cancelSecs || 0) / 120));

  const pickupLat = safeNum(order?.pickup?.lat);
  const pickupLng = safeNum(order?.pickup?.lng);
  const dropLat   = safeNum(order?.drop?.lat);
  const dropLng   = safeNum(order?.drop?.lng);
  const mapRegion = buildTrackingRegion(riderCoords, pickupLat, pickupLng, dropLat, dropLng);
      ...
      <Animated.View style={[S.sheet, { transform: [{ translateY: slideAnim }] }]}>
        {showMap && (
          <View style={S.mapWrap}>
            <SafeMap
              mapRef={mapRef} mapRegion={mapRegion}
              pickupLat={pickupLat} pickupLng={pickupLng}
              dropLat={dropLat} dropLng={dropLng}
              riderCoords={riderCoords}
              onMapReady={() => { if (mountedRef.current) setMapReady(true); }}
            />
        ...
        <View style={[S.card, { paddingBottom: insets.bottom + 16 }]}>
          <View style={S.handle} />
          {loading ? (
            <View style={S.loadingWrap}><ActivityIndicator color={BLUE} size="large" /><Text style={S.loadingText}>Loading your order...</Text></View>
          ) : (
            <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
                  ...
                  {STATUS_STEPS.slice(0, -1).map((s, i) => {
                    ...
                    return (
                      <React.Fragment key={s}>
                        ...
                        {i < STATUS_STEPS.length - 2 && <View style={[S.stepLine, { backgroundColor: i < stepIdx ? info.color : '#E5E7EB' }]} />}
                      </React.Fragment>
                    );
                  })}
                </View>
              ...
              {order?.riderId && status !== 'searching' && (
                <View style={S.riderRow}>
                  <LinearGradient colors={['#0A2F9A', BLUE]} style={S.riderAvatar}>
                    <Text style={S.riderAvatarText}>{(order?.riderId?.name ?? '?').charAt(0).toUpperCase()}</Text>
                  </LinearGradient>
                  <View style={S.riderInfo}>
                    <Text style={S.riderName}>{order?.riderId?.name ?? 'Rider'}</Text>
                    <Text style={S.riderVehicle}>{[order?.riderId?.vehicle?.color, order?.riderId?.vehicle?.model, order?.riderId?.vehicle?.plate ? ('· ' + order.riderId.vehicle.plate) : ''].filter(Boolean).join(' ')}</Text>
                  </View>
                  {(order?.riderId?.rating ?? 0) > 0 && (
                    <View style={S.ratingBadge}>
                      <Ionicons name="star" size={12} color={AMBER} />
                      <Text style={S.ratingText}>{Number(order.riderId.rating).toFixed(1)}</Text>
                    </View>
                  )}
                </View>
              )}
              ...
              <View style={S.routeCard}>
                <View style={S.routeRow}><View style={[S.routeDot, { backgroundColor: GREEN }]} /><Text style={S.routeText} numberOfLines={1}>{order?.pickup?.address ?? ''}</Text></View>
                <View style={S.routeArrow}><Ionicons name="arrow-down" size={14} color="#CBD5E1" /></View>
                <View style={S.routeRow}><View style={[S.routeDot, { backgroundColor: RED }]} /><Text style={S.routeText} numberOfLines={1}>{order?.drop?.address ?? ''}</Text></View>
              </View>

              <View style={S.fareRow}>
                <Text style={S.fareLabel}>Total Fare</Text>
                <Text style={[S.fareValue, { color: BLUE }]}>{fmtCurrency(order?.fare ?? 0)}</Text>
              </View>

              {canCancel && (
                <View style={S.cancelCard}>
                  <View style={S.cancelTop}>
                    <View><Text style={S.cancelTitle}>Cancel order?</Text><Text style={S.cancelSub}>{`Window closes in ${cancelSecs}s`}</Text></View>
                    <Text style={[S.cancelTimer, { color: cancelSecs > 60 ? GREEN : cancelSecs > 20 ? AMBER : RED }]}>{`${cancelSecs}s`}</Text>
                  </View>
                  <View style={S.cancelBar}>
                    <View style={[S.cancelBarFill, { width: `${(cancelProg * 100).toFixed(1)}%`, backgroundColor: cancelSecs > 60 ? GREEN : cancelSecs > 20 ? AMBER : RED }]} />
                  </View>
                  <TouchableOpacity style={[S.cancelBtn, cancelling && { opacity: 0.7 }]} onPress={handleCancel} disabled={cancelling}>
              ...
              {status === 'cancelled' && (
                <View style={S.endCard}>
                  ...
                  <TouchableOpacity style={[S.endBtn, { backgroundColor: RED }]} onPress={onClose}><Text style={S.endBtnText}>Close</Text></TouchableOpacity>
                </View>
              )}
              ...
              {status === 'delivered' && (
                <View style={S.endCard}>
                  ...
                  <TouchableOpacity style={[S.endBtn, { backgroundColor: BLUE }]} onPress={() => { onClose(); try { navigation?.navigate('OrderDetail', { orderId }); } catch (_) {} }}>
              ...
              {!isDone && (
                <TouchableOpacity style={S.viewDetailsBtn} onPress={() => { onClose(); try { navigation?.navigate('OrderDetail', { orderId }); } catch (_) {} }}>
...
const S = StyleSheet.create({
  ...
  card:         { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingTop: 12, paddingHorizontal: 20, marginTop: SCREEN_H * 0.48, minHeight: SCREEN_H * 0.52, elevation: 20, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.15, shadowRadius: 12 },
